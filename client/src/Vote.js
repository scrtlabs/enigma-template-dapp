...
class Vote extends Component {
   ...
   */
  vote(event) {
    var BN = this.props.enigmaSetup.web3.utils.BN;
    ...
    this.props.voting
      .castVote(
        parseInt(this.votePollID.value),
        encryptedVote,
        this.props.enigmaSetup.web3.utils.toWei(this.curWeight.value, "ether"),
        {
      ...
      .catch(error => {
        this.alert.current.openAlert(
          "Unable to cast vote. Either you have already voted, the poll has ended, or your parameters are invalid."
        );
        console.log(error);
      });
  }
        ...
        <List component="nav" disablePadding={true}>
          <ListItem>
            <form onSubmit={this.vote} id="vote_form">
              <label>
                {" "}
                Enter 0 to cast a "no" vote and enter 1 to cast a "yes" vote.
                Weight refers to the number of voting credits that you will use.
              </label>{" "}
              <br /> <br />
              Vote:
              <TextField
                ...
                placeholder="Value"
                inputRef={element => (this.curVote = element)}
              />
              <TextField
                className={classes.textField}
                placeholder="Weight"
                inputRef={element => (this.curWeight = element)}
              />
              <TextField
                className={classes.textField}
                placeholder="Poll ID"
                inputRef={element => (this.votePollID = element)}
              />
              <Button
